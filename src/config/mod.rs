...
fn check_global_config_keys(value: &toml::Value, file_path: &str) {
...
fn check_project_config_keys(value: &toml::Value, file_path: &str) {
    let Some(table) = value.as_table() else {
        return;
    };
...
pub fn load_global_config() -> Result<GlobalConfig> {
    ...
    if let Ok(raw) = contents.parse::<toml::Value>() {
        check_global_config_keys(&raw, &config_path.to_string_lossy());
    }
...
pub fn load_project_config(repo_root: &Path) -> Result<Option<ProjectConfig>> {
    ...
    // Check for unknown keys before deserializing
    if let Ok(raw) = contents.parse::<toml::Value>() {
        check_project_config_keys(&raw, &config_path.to_string_lossy());
    }
...
mod tests {
    ...
    fn agent_build_command_codex() {
        ...
        // Codex always uses --full-auto (built-in sandbox)
    ...
    fn agent_build_command_aider() {
        ...
        // Default: interactive, no auto-approve
        assert_eq!((caps.build_command)(None, false, false), "aider");
        // Unrestricted: --yes auto-approves
    ...
    fn agent_build_command_opencode() {
        ...
        // OpenCode has no CLI auto-approve flag
    ...
    fn check_global_config_keys_no_warnings_for_valid() {
        ...
        // Should not panic or produce warnings (we can't capture stderr easily,
    ...
    fn check_global_config_keys_detects_unknown() {
        ...
        "#;
        let value: toml::Value = toml_str.parse().unwrap();
        // This will print a warning to stderr — verify it doesn't panic
        check_global_config_keys(&value, "test");
    }
    ...
    fn check_project_config_keys_detects_unknown_in_pane_override() {
        ...
        "#;
        let value: toml::Value = toml_str.parse().unwrap();
        // split_from is valid in PaneConfig but NOT in PaneOverride
        check_project_config_keys(&value, "test");
    }
    ...
    fn warn_unknown_keys_finds_extras() {
        ...
        // Verify it doesn't panic — the warning goes to stderr
